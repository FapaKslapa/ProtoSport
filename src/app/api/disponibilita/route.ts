...
export async function GET() {
    try {
        ...
        return NextResponse.json({success: false, error: 'Errore nel recupero delle disponibilità'}, {status: 500});
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (esistente) {
            db.prepare('UPDATE disponibilita SET ora_inizio = ?, ora_fine = ? WHERE giorno_settimana = ?')
                .run(ora_inizio, ora_fine, giorno_settimana);
            return NextResponse.json({success: true, data: {id: esistente.id, giorno_settimana, ora_inizio, ora_fine}});
        } else {
            const result = db.prepare('INSERT INTO disponibilita (giorno_settimana, ora_inizio, ora_fine) VALUES (?, ?, ?)')
                .run(giorno_settimana, ora_inizio, ora_fine);
            return NextResponse.json({
                success: true,
        ...
        return NextResponse.json({success: false, error: 'Errore nella creazione della disponibilità'}, {status: 500});
    }
}