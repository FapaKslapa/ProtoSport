...
// Funzione per generare il contenuto ICS
function prenotazioneToICS(p: Prenotazione) {
    // Data e ora in formato YYYYMMDDTHHMMSS
...
const PrenotazioniUtenteModal: React.FC<Props> = ({prenotazioni, onClose, isOpen}) => {
    const today = formatDate(new Date());
    const [selectedDate, setSelectedDate] = useState<string>(today);

    // Trova tutte le date future (comprese oggi) con prenotazioni
    ...
    const prenotazioniDelGiorno = prenotazioni.filter(p => p.data_prenotazione === selectedDate);
                    ...
                    <ul className="space-y-4">
                        {prenotazioniDelGiorno.map(p => (
                            <li key={p.id}
                                ...
                                ${selectedDate === today ? 'border-red-500 bg-red-50' : 'border-gray-200 bg-white'}`}>
                                <div className="flex justify-between items-center">
                                    <span className="font-semibold text-black">{p.servizio_nome}</span>
                                    <span className={`text-xs px-2 py-1 rounded-full
                                        ${selectedDate === today ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'}`}>
