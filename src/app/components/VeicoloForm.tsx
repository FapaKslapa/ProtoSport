...
export default function VeicoloForm({onSave}: VeicoloFormProps) {
    ...
    const anni = (marca && modello && catalogo[marca]?.[modello]?.anni)
        ? [...catalogo[marca][modello].anni].sort((a, b) => b - a)
        : [];

    const cilindrate = (marca && modello && catalogo[marca]?.[modello]?.cilindrate)
